...
def _build_payload(event: SlackEvent) -> dict:
    ...
    if event.session_id:
        fields.append({"title": "Session", "value": event.session_id[:8] + "…", "short": True})
    if event.run_id:
    ...
    if event.adapter_version:
        fields.append({"title": "Adapter", "value": event.adapter_version, "short": True})
    if event.s3_path:
...
def send(event: SlackEvent, retries: int = 3, backoff: float = 1.5) -> bool:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            logger.info("slack_sent", extra={"stage": event.stage, "status": event.status})
            return True
            ...
            if attempt == retries:
                logger.error("slack_failed", extra={"stage": event.stage, "error": str(exc)})
                return False
...
def session_started(session_id: str) -> None:
    send(SlackEvent(
        stage="session_started", status="info",
...
def pre_sleep_warning(session_id: str, tokens_remaining: int) -> None:
    send(SlackEvent(
        stage="pre_sleep_warning", status="warn",
...
def session_sleeping(session_id: str) -> None:
    send(SlackEvent(
        stage="session_sleeping", status="info",
...
def artifact_uploaded(run_id: str, artifact_type: str, s3_path: str) -> None:
    send(SlackEvent(stage="artifact_uploaded", status="info",
        summary=f"Artifact uploaded: {artifact_type}",
        run_id=run_id, s3_path=s3_path))


def training_started(run_id: str, session_id: str, hf_job_id: str) -> None:
    send(SlackEvent(stage="training_started", status="info",
...
def training_succeeded(run_id: str, s3_path: str) -> None:
    send(SlackEvent(stage="training_succeeded", status="ok",
        summary="Training job completed successfully.",
        run_id=run_id, s3_path=s3_path))


def training_failed(run_id: str, error: str) -> None:
    send(SlackEvent(stage="training_failed", status="error",
        summary=f"Training job failed: {error}",
        run_id=run_id, extra={"error": error}))


def evaluation_started(run_id: str) -> None:
    send(SlackEvent(stage="evaluation_started", status="info",
        summary="Running evaluation suite.", run_id=run_id))


def evaluation_completed(run_id: str, passed: bool, score: float, s3_path: str) -> None:
    send(SlackEvent(stage="evaluation_completed",
